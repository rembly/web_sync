...
class SalesforceZoomSync
  ...
  def update_zoom_attendees(participants, call_date, call_type, date_field, only_new = false)
    ...
    to_update = only_new ? matched_email.select { |c| c.send(date_field).blank? || c.send(date_field).to_date < call_date } : matched_email
    log_sf_update(to_update, call_type, call_date)
    to_update.each { |user| @sf.set_contact_date(contact: user, date: call_date, date_field: date_field) } if to_update.try(:any?)
    # return all matched, not just updated
    matched_email
  end
  ...
  def update_zoom_callers(participants, call_date, call_type, date_field, only_new = false)
    ...
    to_update = only_new ? matched_phone.select { |c| c.send(date_field).blank? || c.send(date_field).to_date < call_date } : matched_phone
    log_sf_update(to_update, call_type, call_date)
    to_update.each { |user| @sf.set_contact_date(contact: user, date: call_date, date_field: date_field) } if to_update.try(:any?)
    matched_phone
  end
  ...
  def log_missed_call(_participants, matched_sf)
  ...
  def log_unmatched_in_sf(matched_sf, zoom_participants, webinar_type)
    ...
    end.each { |unmatched_intro_attendee| log(zoom_user_print(unmatched_intro_attendee)) }
  end
  ...
  def phone_match?(sf_phone, zoom_phone)
    sf_num = sf_phone.to_s.gsub(/[^\d]/, '')
    sf_num.present? && (sf_num.include?(zoom_phone.to_s) || zoom_phone.to_s.include?(sf_num))
  end
